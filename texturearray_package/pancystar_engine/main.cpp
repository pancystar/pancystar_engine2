...
HRESULT engine_windows_main::game_create()
{
	...
	//创建d3d初始化
	engine_basic::engine_fail_reason fail_message = d3d_pancy_basic_singleton::single_create(hwnd, window_width, window_hight);
	...
	//创建物理系统
	fail_message = pancy_physx_basic::single_create();
	if (!fail_message.check_if_failed())
	{
		fail_message.show_failed_reason();
		return E_FAIL;
