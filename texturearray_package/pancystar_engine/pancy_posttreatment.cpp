...
engine_basic::engine_fail_reason render_posttreatment_RTGR::build_texture()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		hr = d3d_pancy_basic_singleton::GetInstance()->get_d3d11_device()->CreateRenderTargetView(final_reflect_data, &rtvDesc, &final_reflectmip_target[i]);
		if (FAILED(hr))
		{
			engine_basic::engine_fail_reason error_message(hr, "create reflect GI mipmap RTV error in globel reflect part");
			return error_message;
		}
	}
	*/
	...
	hr = d3d_pancy_basic_singleton::GetInstance()->get_d3d11_device()->CreateRenderTargetView(blur_reflect_data2, NULL, &blur_reflect_target2);
	if (FAILED(hr))
	{
		engine_basic::engine_fail_reason error_message(hr, "create mid reflect blur RTV error in globel reflect part");
		return error_message;
...
void render_posttreatment_RTGR::release_texture()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		safe_release(final_reflectmip_target[i]);
	}*/
